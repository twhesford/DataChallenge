...
def wvstorm_table(buoy,bot_strs,thresh,dur,end_thresh,end_dur,wndbuoy,
                  cur_dir=None,cur_strs=None,roughness=.000675,Nstorms=None,storm_names=None):
    """
    docstring"""
    ...
    def avg_angles(angles,storm):
        ...
        ang = np.around(np.rad2deg(np.arctan2(y,x)),2)
    ...
    ubr,T_b = buoy.waves.bottom_velocity(buoy.waves.depth)
    
    
#     if cur_dir is None or cur_strs is None:
#         bot_strs = buoy.waves.bottom_stress(buoy.waves.depth,roughness)[0]
#     else:
#         bot_strs = buoy.waves.rep_cur_stress(cur_dir,cur_strs,buoy.waves.depth,roughness)[0]
        
    int_strs = np.array(pd.DataFrame(bot_strs).interpolate())[:,0]
    wv_storms = storm_search(int_strs,thresh,dur,end_thresh,end_dur)
    Idata,ordered_storms = integrate_storms(bot_strs,int_strs,wv_storms,buoy.timestamps)
    if Nstorms is None:
        Nstorms = len(Idata)
    if storm_names is None:
        storm_names = ['']*len(Idata)
    
    peak_strs = list(map(lambda storm : np.around(np.nanmax(bot_strs[storm[0]:storm[1]]),2) if len(storm) > 0 else 0,ordered_storms))
    ...
    mean_strs,mean_ubr,mean_swh,mean_tp,mean_mwd,mean_wndstrs,mean_wnddir,I = [ [] for i in range(8)]
    i=0
    for storm in ordered_storms:
        I.append(round(Idata[i],2))
        mean_strs.append(np.around(storm_mean(storm,bot_strs),2))
        mean_swh.append(np.around(storm_mean(storm,buoy.waves.swh),2))
        mean_tp.append(np.around(storm_mean(storm,buoy.waves.Tp),2))
        mean_wndstrs.append(np.around(storm_mean(storm,wnd_strs),2))
        mean_ubr.append(np.around(storm_mean(storm,ubr),2))
    ...
    return table.round(2),ordered_storms

def wndstorm_table(buoy,thresh,dur,end_thresh,end_dur,wvbuoy,Nstorms=None,storm_names=None):
    ...
    Idata,ordered_storms = integrate_storms(wnd_strs,int_strs,wnd_storms,buoy.timestamps)
    ...
    for storm in ordered_storms:
        ...
        mean_Tp.append(storm_mean(storm,wvbuoy.waves.Tp))
...
def warner_class(buoy,storms,low_thresh,window=28):
    ...
    def check_wnddir(buoy,storm,low_idx=None,window=28):  
        if low_idx is None:
            start_mwd = avg_angles(buoy.wind.j,[storm[0],(storm[1]-storm[0])//2])
            end_mwd = avg_angles(buoy.wind.j,[(storm[1]-storm[0])//2,storm[1]])
        else:
    ...
    def check_temp(buoy,low_idx,storm,window=28):
        
        pre_grad = np.gradient(buoy.climate.air_temp[low_idx-window:low_idx])
        post_grad = np.gradient(buoy.climate.air_temp[low_idx:low_idx+window])
    
        if np.nanmean(pre_grad) > 0 and np.nanmean(post_grad) < 0:
            return 'cold'
        else:
            return check_wnddir(buoy,storm,low_idx=low_idx,window=window)
            
    idx_map = np.array(range(len(buoy.climate.atm_pressure)),dtype=int)
    classes = []
    for strm in storms:
        min_prs = np.nanmin(buoy.climate.atm_pressure[strm[0]:strm[1]])
       
        if min_prs < low_thresh:
            
            low_idx = list(buoy.climate.atm_pressure[strm[0]:strm[1]]).index(min_prs) + idx_map[strm[0]]
            classes.append(check_temp(buoy,low_idx,strm,window=window))
        else:
            classes.append(check_wnddir(buoy,strm,window=window))
