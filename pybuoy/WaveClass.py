...
class Waves(Vector2d):
    ...
    """Make a spec class and a bulk class and give wave class inheritence making the funcitons here minimal"""
    """
    ...
    def __init__(self,i=None,j=None,swh=None, k=None,depth=None,Tm=None,Tp=None,spec=None,fbins=None):
        super().__init__(i,j)
        self.swh,self.k,self.Tp,self.Tm,self.depth,self.spec,self.fbins = swh,k,Tp,Tm,depth,spec,fbins
...
#         ret True
    ...
    def energy(self,swdens=1025):
        return (swdens/8) * 9.81 * self.swh**2 
    ...
    def bottom_friction(self,u_b,T_b,K_N,C_mu = 1):
    ...
    def bottom_stress(self,h,K_N,C_mu=1,rho=1025):
    
        u_b,T_b = self.bottom_velocity(h)
        f_w = self.bottom_friction(u_b,T_b,K_N,C_mu=C_mu)

        return rho*f_w*(u_b**2),f_w
    
    def rep_stress(self,cur_dir,cur_strs,h,K_N,rho=1025):

        phi =  np.deg2rad(cur_dir - self.j)
        cur_shear = np.sqrt(cur_strs/rho)

        mu = 0
        C_mu = 1
        wv_strs,f_w = self.bottom_stress(h,K_N,C_mu=C_mu,rho=rho)
        ...
        while check == False:
            ...
            wv_strs,new_f_w = self.bottom_stress(h,K_N,C_mu=C_mu,rho=rho)
    ...
    def rep_cur_stress(self,cur_dir,cur_vel,z_r,h,K_N,rho=1025):
        ...
        while check == False:
            ...
            C_mu = np.sqrt(1 + 2*mu*np.abs(np.cos(phi)) + mu**2)
            new_f_w = self.bottom_friction(u_b,t_b,K_N,C_mu=C_mu)
            dif = np.abs(f_w-new_f_w)/new_f_w
